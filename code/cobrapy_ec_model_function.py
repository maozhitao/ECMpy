    ...
    *reaction_mw: The molecular weight of the enzyme that catalyzes
    ...
    for reaction_idmw in reaction_mw.index:
        ...
        if reaction_id in reaction_kcat.index:
            ...
            reaction_kcat_mw.loc[reaction_idmw,
                                 'kcat'] = reaction_kcat.loc[reaction_id, 'kcat']
    ...
    for r in norm_model.reactions:
        with norm_model as model:
            if r.id in list(reaction_kcat_mw.index):
                r.bounds = (
                    0, reaction_kcat_mw.loc[r.id, 'kcat_MW']*enzyme_amount*percentage)
                df_biomass.loc[r.id, 'biomass'] = model.slim_optimize()
                biomass_diff = norm_biomass-model.slim_optimize()
                biomass_diff_ratio = (
                    norm_biomass-model.slim_optimize())/norm_biomass
                ...
                if biomass_diff_ratio > select_value:  # select difference range
                    ...
                    df_biomass_select.loc[r.id,
                                          'biomass_diff_ratio'] = biomass_diff_ratio

    df_biomass = df_biomass.sort_values(
        by="biomass_diff_ratio", axis=0, ascending=False)
    df_biomass.to_csv(reaction_biomass_outfile)

    if df_biomass_select.empty:
        pass
    else:
        df_reaction_select = df_biomass_select.sort_values(
            by="biomass_diff_ratio", axis=0, ascending=False)
...
def calibration_kcat(need_change_reaction, reaction_kcat_mw_file, json_model_path, adj_kcat_title, change_kapp_file, reaction_kapp_change_file):
    ...
    for eachreaction in need_change_reaction:
        ...
        reaction_kappori.loc[eachreaction, 'kcat_MW'] = reaction_kappori.loc[eachreaction,
                                                                             'kcat'] / reaction_kappori.loc[eachreaction, 'MW']
        for r in norm_model.reactions:
            with norm_model as model:
                if r.id == eachreaction:
                    r.bounds = (
                        0, reaction_kappori.loc[eachreaction, 'kcat_MW']*0.0228)
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'kcat_ori'] = kcat_ori
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'kcat_change'] = reaction_kappori.loc[eachreaction, 'kcat']
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'MW'] = reaction_kappori.loc[eachreaction, 'MW']
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'kcat_mw_new'] = reaction_kappori.loc[eachreaction, 'kcat_MW']
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'norm_biomass'] = norm_biomass
                    round_1_reaction_kapp_change.loc[eachreaction,
                                                     'new_biomass'] = model.slim_optimize()
...
def change_reaction_kcat_by_fold(select_reaction, change_fold, reaction_kcat_mw_file, reaction_kapp_change_file):
    ...
    for eachreaction in select_reaction:
        reaction_kcat_mw.loc[eachreaction,
                             'kcat'] = reaction_kcat_mw.loc[eachreaction, 'kcat'] * change_fold
        reaction_kcat_mw.loc[eachreaction,
                             'kcat_MW'] = reaction_kcat_mw.loc[eachreaction, 'kcat_MW'] * change_fold
...
def change_reaction_kcat_by_foldlist(select_reaction, change_fold, reaction_kcat_mw_file, reaction_kapp_change_file):
    ...
    for eachreaction in select_reaction:
        reaction_kcat_mw.loc[eachreaction,
                             'kcat'] = reaction_kcat_mw.loc[eachreaction, 'kcat'] * change_fold[i]
        reaction_kcat_mw.loc[eachreaction,
                             'kcat_MW'] = reaction_kcat_mw.loc[eachreaction, 'kcat_MW'] * change_fold[i]
        reaction_change_accord_fold.append(eachreaction)
        i = i+1
    reaction_kcat_mw.to_csv(reaction_kapp_change_file)
    return(reaction_change_accord_fold)
...
def draw_calibration_kcat_figure(Orimodel_solution_frame, ECMpy_solution_frame, ECMpy_adj_solution_frame, insvg, outsvg):
    ...
    for reaction in ECMpy_adj_solution_frame.index:
        if re.search('_forward', reaction):
            eachreaction = reaction.split('_forward')[0]
            if eachreaction in flux2_list.index:
                flux2_list.loc[eachreaction, 'fluxes'] = np.max(
                    [flux2_list.loc[eachreaction, 'fluxes'], ECMpy_solution_frame.loc[reaction, 'fluxes']])
            else:
                flux2_list.loc[eachreaction,
                               'fluxes'] = ECMpy_solution_frame.loc[reaction, 'fluxes']
            if eachreaction in flux3_list.index:
                flux3_list.loc[eachreaction, 'fluxes'] = np.max(
                    [flux3_list.loc[eachreaction, 'fluxes'], ECMpy_adj_solution_frame.loc[reaction, 'fluxes']])
            else:
                flux3_list.loc[eachreaction,
                               'fluxes'] = ECMpy_adj_solution_frame.loc[reaction, 'fluxes']
        elif re.search('_num', reaction):
            ...
            flux2_list.loc[reaction,
                           'fluxes'] = ECMpy_solution_frame.loc[reaction, 'fluxes']
            flux3_list.loc[reaction,
                           'fluxes'] = ECMpy_adj_solution_frame.loc[reaction, 'fluxes']
        ...
        with norm_model as model:
            if r.id in list(reaction_kcat_mw.index):
                ...
                if model.slim_optimize() < select_value*norm_biomass:  # select difference range
                    df_biomass_select.loc[r.id,
                                          'biomass_enz'] = model.slim_optimize()
                    df_biomass_select.loc[r.id,
                                          'biomass_orimodel'] = norm_biomass
                    df_biomass_select.loc[r.id, 'biomass_diff'] = biomass_diff
                    df_biomass_select.loc[r.id,
                                          'biomass_diff_ratio'] = biomass_diff_ratio
...
def get_enz_model_use_biomass_diff(reaction_kcat_mw_file, json_model_path, percentage, reaction_biomass_outfile, select_percentage, kcat_data_colect_file, model_file, f, ptot, sigma, lowerbound, upperbound, json_output_file, reaction_mw_outfile):
    df_reaction_select = select_calibration_reaction_by_biomass(
        reaction_kcat_mw_file, json_model_path, upperbound, percentage, reaction_biomass_outfile, select_percentage)
    if isinstance(df_reaction_select, pd.DataFrame):
        ...
        change_reaction_list_round1 = change_reaction_kcat_by_database(
            need_change_reaction, kcat_data_colect_file, reaction_kcat_mw_file, reaction_kapp_change_file)
...
def get_enz_model_use_c13(reaction_kcat_mw_file, c13reaction_file, percentage, df_reaction_select, kcat_data_colect_file, model_file, f, ptot, sigma, lowerbound, upperbound, json_output_file, reaction_mw_outfile):
    ...
    change_reaction_list_round1 = change_reaction_kcat_by_database(
        c13reaction_selecet, kcat_data_colect_file, reaction_kcat_mw_file, reaction_kapp_change_file)
    print(change_reaction_list_round1)
...
def get_enz_model_use_enz_usage(enz_ratio, reaction_flux_file, reaction_kcat_mw_file, reaction_enz_usage_file, kcat_data_colect_file, model_file, f, ptot, sigma, lowerbound, upperbound, json_output_file, reaction_mw_outfile):
    ...
    reaction_kapp_change_file = reaction_mw_outfile
    change_reaction_list_round1 = change_reaction_kcat_by_database(
        select_reaction, kcat_data_colect_file, reaction_kcat_mw_file, reaction_kapp_change_file)
    print(change_reaction_list_round1)
